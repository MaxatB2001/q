...
export class TaskComponent implements OnInit {
  ...
  constructor(private el: ElementRef, private ref: ChangeDetectorRef, private taskService: TaskService) {}
  ...
  mouseUpHandler = ($event: MouseEvent) => {
    ...
    const prevLeft = Number(
      this.oldLeft.substring(0, this.oldLeft.length - 2)
    );
  ...
  incrTaskDate = (diff:number) => {
    ...
    if (steps == 0) {
      this.el.nativeElement.children[0].style.marginLeft = parseInt(this.el.nativeElement.children[0].style.marginLeft) - diff + 'px'
      return
    }
  ...
  calculateTask = (newTaskStart: Date, newTaskDeadline: Date) => {
    ...
    this.taskService.updateTask(this.task.id as number, {created_at: newTaskStart, deadline: newTaskDeadline}).subscribe(data => console.log(data))
